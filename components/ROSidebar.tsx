...
const navItems = [
  { href: '/ro-dashboard', label: 'Dashboard', icon: '⊞' },
...
export default function ROSidebar() {
  ...
  const [user, setUser] = useState<{ name: string; role: string; district?: string } | null>(null);
  ...
  return (
    <aside style={{ width: '220px', minWidth: '220px', background: 'white', borderRight: '2px solid #c8e0d2', display: 'flex', flexDirection: 'column', height: '100vh', position: 'sticky', top: 0, boxShadow: '2px 0 8px rgba(26,107,58,0.07)' }}>
      ...
      <div style={{ background: '#f0f0ff', borderBottom: '1px solid #c8d0f0', padding: '0.4rem 1rem' }}>
        <span style={{ fontSize: '0.62rem', fontWeight: '700', color: '#2d6a9f', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Regional Officer</span>
        {user?.district && <div style={{ fontSize: '0.6rem', color: '#4a6a8a', marginTop: '0.1rem' }}>Zone: {user.district}</div>}
      </div>
